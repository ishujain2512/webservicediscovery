...
def create_excel(categories_data):
    wb = Workbook()
    for category, data in categories_data.items():
        ws = wb.create_sheet(title=category)
        ws.append(["Category", "Name", "Link",
        ...
        for line in data:
            if line.strip().startswith("|"):
                ...
                if len(parts) >= 5:
                    name_link = parts[1].strip()
                    name = strip_square_brackets(name_link)
                    url = extract_url(name_link)
                    description = parts[2]
                    auth = parts[3]
                    https = parts[4]
                    cors = parts[5]
                    ws.append(
                        [category, name, url, description, auth, https, cors])

    wb.save("animal_anime_apis.xlsx")


def strip_square_brackets(name_link):
    # Extract the text within square brackets
    if "[" in name_link and "]" in name_link:
        return name_link.split("[")[1].split("]")[0].strip()
    else:
        return name_link.strip()


def extract_url(name_link):
    # Find the last occurrence of '(' and ')' to handle cases where '(' or ')' may be part of the URL
    last_opening_bracket = name_link.rfind('(')
    last_closing_bracket = name_link.rfind(')')

    # If both '(' and ')' are found, extract the URL accordingly
    if last_opening_bracket != -1 and last_closing_bracket != -1:
        url = name_link[last_opening_bracket + 1:last_closing_bracket].strip()
    else:
        # If either '(' or ')' is not found or if the format is not as expected, return an empty string
        url = ""

    return url


def extract_categories_and_data(text):
    categories_data = {}
    ...
    for line in text.splitlines():
        ...
        if line.startswith("###"):
            if current_category:
                categories_data[current_category] = current_data
            current_category = line.split("###")[1].strip()
            current_data = []
        elif line and not line.startswith("|---|") and not line.startswith("**[⬆ Back to Index](#index)**"):
...
| [Yandex.Weather](https://yandex.com/dev/weather/) | Assesses weather condition in specific locations | `apiKey` | Yes | No |
    ...
    categories_data = extract_categories_and_data(data)
    create_excel(categories_data)
